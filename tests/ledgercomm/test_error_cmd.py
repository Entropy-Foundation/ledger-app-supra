...
def test_bad_cla(cmd):
    sw, _ = cmd.transport.exchange(
        cla=0xA0, ins=0x03, p1=0x00, p2=0x00, cdata=b""  # 0xa0 instead of 0x5b
    )
...
def test_bad_ins(cmd):
    sw, _ = cmd.transport.exchange(
        cla=0x5B, ins=0xFF, p1=0x00, p2=0x00, cdata=b""  # bad INS
    )
...
def test_wrong_p1p2(cmd):
    sw, _ = cmd.transport.exchange(
        cla=0x5B, ins=0x03, p1=0x01, p2=0x00, cdata=b""  # 0x01 instead of 0x00
    )
...
def test_wrong_data_length(cmd):
    ...
    sw, _ = cmd.transport.exchange_raw("5B00")
